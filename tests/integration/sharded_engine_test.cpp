...
InstrumentRegistry make_registry(std::uint32_t n) {
    ...
    for (std::uint32_t i = 1; i <= n; ++i) {
        r.add(InstrumentSpec{InstrumentId{i}, "S" + std::to_string(i),
                             InstrumentType::Equity, 1, 1, "USD"});
    }
    r.freeze();
    return r;
}
...
TEST(ShardedEngine, ConsistentHashingAssignsInstruments) {
    ...
    // Every instrument's book lives on exactly one shard.
...
TEST(ShardedEngine, SingleSubmitMatches) {
    ...
    ShardedEngine eng{r, 2};
    EXPECT_TRUE(eng.submit(mk(1, InstrumentId{1}, Side::Buy, 100, 5)));
    eng.wait_idle();
...
TEST(ShardedEngine, CrossMatchesAcrossShards) {
    ...
    eng.submit(mk(2, InstrumentId{2}, Side::Sell, 200, 5));
    eng.submit(mk(3, InstrumentId{1}, Side::Buy, 100, 5));
    eng.submit(mk(4, InstrumentId{2}, Side::Buy, 200, 5));
    eng.wait_idle();
...
TEST(ShardedEngine, ManyOrdersPreserveCount) {
    ...
    for (std::uint32_t inst = 1; inst <= 4; ++inst) {
        for (std::uint64_t i = 0; i < kPerInst; ++i) {
            ...
            eng.submit(mk(next_id++, InstrumentId{inst}, Side::Buy, price, 1));
        }
    }
    eng.wait_idle();
    for (std::uint32_t inst = 1; inst <= 4; ++inst) {
...
TEST(ShardedEngine, SingleShardWorks) {
    ...
    eng.submit(mk(1, InstrumentId{1}, Side::Sell, 100, 5));
    eng.submit(mk(2, InstrumentId{1}, Side::Buy, 100, 5));
    eng.wait_idle();
    EXPECT_TRUE(eng.book(InstrumentId{1})->empty());
}
