...
bool encode_header(BufferWriter& w, const MessageHeader& h) noexcept {
    return w.write_u8(h.protocol_version)
        && w.write_u8(static_cast<std::uint8_t>(h.msg_type))
        && w.write_u16(h.body_length)
        && w.write_u32(h.sequence);
}

std::optional<MessageHeader> decode_header(BufferReader& r) noexcept {
    MessageHeader h{};
    std::uint8_t mt = 0;
    if (!r.read_u8(h.protocol_version)) return std::nullopt;
    if (!r.read_u8(mt))                 return std::nullopt;
    if (!r.read_u16(h.body_length))     return std::nullopt;
    if (!r.read_u32(h.sequence))        return std::nullopt;
    ...
    return std::visit([&](const auto& m) -> bool {
        ...
        if constexpr (std::is_same_v<T, LogonMsg>) {
            ...
            return w.write_u64(m.client_order_id)
                && w.write_u32(m.instrument_id)
                && w.write_u8(m.side)
                && w.write_u8(m.order_type)
                && w.write_u8(m.tif)
                && w.write_u8(0)    // padding to 8-byte alignment
                && w.write_i64(m.price)
                && w.write_u64(m.quantity);
            ...
            return w.write_u64(m.client_order_id)
                && w.write_u64(m.server_order_id)
                && w.write_i64(m.price)
                && w.write_u64(m.quantity)
                && w.write_u8(m.post_status);
...
[[nodiscard]] MsgType type_of(const MessageBody& body) noexcept {
    return std::visit([](const auto& m) -> MsgType {
        using T = std::decay_t<decltype(m)>;
        if constexpr (std::is_same_v<T, LogonMsg>)        return MsgType::Logon;
        else if constexpr (std::is_same_v<T, LogoutMsg>)  return MsgType::Logout;
        else if constexpr (std::is_same_v<T, HeartbeatMsg>) return MsgType::Heartbeat;
        else if constexpr (std::is_same_v<T, NewOrderMsg>)  return MsgType::NewOrder;
        else if constexpr (std::is_same_v<T, CancelOrderMsg>) return MsgType::CancelOrder;
        else if constexpr (std::is_same_v<T, OrderAckMsg>)  return MsgType::OrderAck;
        else if constexpr (std::is_same_v<T, OrderRejectMsg>) return MsgType::OrderReject;
        else if constexpr (std::is_same_v<T, FillMsg>)     return MsgType::Fill;
        else                                               return MsgType::Cancelled;
    }, body);
}
...
std::size_t encode(std::span<std::byte> out, std::uint32_t seq, const MessageBody& body) {
    ...
    if (!encode_body(bw, body)) return 0;
    const std::size_t body_len = bw.pos();
    if (out.size() < kHeaderSize + body_len) return 0;
    if (body_len > kMaxBodySize) return 0;
    ...
    BufferWriter hw{out};
    if (!encode_header(hw, h)) return 0;
...
decode_body(const MessageHeader& header, std::span<const std::byte> body) noexcept {
    ...
    switch (header.msg_type) {
        case MsgType::Logon: {
            LogonMsg m{};
            if (!r.read_u32(m.client_id)) return std::nullopt;
            return MessageBody{m};
        }
        case MsgType::Logout:    return MessageBody{LogoutMsg{}};
        case MsgType::Heartbeat: return MessageBody{HeartbeatMsg{}};
        case MsgType::NewOrder: {
            NewOrderMsg m{};
            std::uint8_t pad = 0;
            if (!r.read_u64(m.client_order_id)) return std::nullopt;
            if (!r.read_u32(m.instrument_id))   return std::nullopt;
            if (!r.read_u8(m.side))             return std::nullopt;
            if (!r.read_u8(m.order_type))       return std::nullopt;
            if (!r.read_u8(m.tif))              return std::nullopt;
            if (!r.read_u8(pad))                return std::nullopt;
            if (!r.read_i64(m.price))           return std::nullopt;
            if (!r.read_u64(m.quantity))        return std::nullopt;
            return MessageBody{m};
        }
        ...
        case MsgType::OrderAck: {
            OrderAckMsg m{};
            if (!r.read_u64(m.client_order_id)) return std::nullopt;
            if (!r.read_u64(m.server_order_id)) return std::nullopt;
            return MessageBody{m};
        }
        case MsgType::OrderReject: {
            ...
            if (!r.read_u64(m.client_order_id)) return std::nullopt;
            if (!r.read_u8(reason))             return std::nullopt;
        ...
        case MsgType::Fill: {
            ...
            if (!r.read_i64(m.price))           return std::nullopt;
            if (!r.read_u64(m.quantity))        return std::nullopt;
            if (!r.read_u8(m.post_status))      return std::nullopt;
            return MessageBody{m};
        }
