...
std::string Contract::occ_symbol() const
{
    ...
    char strike_str[9];
    std::snprintf(strike_str, sizeof(strike_str), "%08lld", static_cast<long long>(thousandths));
...
Contract Contract::from_occ(std::string_view occ)
{
    ...
    c.expiry = ExpiryDate{static_cast<std::uint16_t>(2000 + yy),
    ...
    if (cp == 'C')
        ...
        throw std::invalid_argument{"OCC symbol: expected 'C' or 'P' at position 12"};
