...
RejectReason classify(OrderStatus s, const NewOrderMsg& m, const Engine& engine)
{
    ...
    if (m.order_type == 0 && m.price <= 0)
        return RejectReason::InvalidPrice;
    if (m.order_type == 1 && (m.tif == 0 || m.tif == 3))
        return RejectReason::InvalidTif;
    if (m.tif == 2)
        return RejectReason::InsufficientLiquidity;
    return RejectReason::Unknown;
}
...
{
    if (s.state() != Session::State::NotLoggedOn)
        return;
