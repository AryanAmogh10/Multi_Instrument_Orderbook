...
void init_once() {
    bool expected = false;
    if (!g_wsa_initialised.compare_exchange_strong(expected, true)) return;
    WSADATA wsa{};
...
void close_socket(socket_t s) noexcept { ::closesocket(s); }
#else
void init_once() {}
void close_socket(socket_t s) noexcept { ::close(s); }
#endif

long recv_some(socket_t s, std::span<std::byte> out) noexcept {
#ifdef _WIN32
    const int n = ::recv(s, reinterpret_cast<char*>(out.data()),
...
socket_t listen_on(std::uint16_t port, int backlog) {
    ...
    if (s == kInvalidSocket) throw std::runtime_error{"socket() failed"};
