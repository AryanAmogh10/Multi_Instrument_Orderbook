...
Client::Client(std::string_view host, std::uint16_t port)
    : sock_(tcp::connect_to(host, port)) {}

Client::~Client() {
    if (sock_ != tcp::kInvalidSocket) tcp::close_socket(sock_);
}
...
std::optional<protocol::DecodedMessage> Client::recv() {
    ...
    while (true) {
        if (auto m = framer_.next()) return m;
        if (framer_.has_error()) return std::nullopt;
